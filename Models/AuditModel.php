<?php
	...
	class AuditModel
	{
		...
		static function fetch_n_most_recent_with_offset($amount, $offset)
		{
			...
			$f = $db->getDBConnection()->prepare("SELECT * FROM (SELECT auditID, INET6_NTOA(ip), URL, timestamp, request FROM audit ORDER BY timestamp DESC LIMIT ? OFFSET ?) please ORDER BY auditID DESC;");
			$f->bind_param("ii", $amount, $offset);
			$f->bind_result($auditID, $ip, $URL, $timestamp, $request);
			$f->execute();
			$f->store_result();
